...
func TestNewDlvSpecDefaults(t *testing.T) {
	spec := newDlvSpec(20)
	expected := dlvSpec{mode: "exec", host: "localhost", port: 20, apiVersion: 2, headless: true, log: false, onlySameUser: false}
...
func TestExtractDlvArg(t *testing.T) {
	tests := []struct {
		...
		{[]string{"foo"}, nil},
		{[]string{"foo", "--foo"}, nil},
		{[]string{"dlv", "debug", "foo"}, &dlvSpec{mode: "debug", headless: false, apiVersion: 2, log: false, onlySameUser: true}},
		{[]string{"dlv", "debug", "--headless"}, &dlvSpec{mode: "debug", headless: true, apiVersion: 2, log: false, onlySameUser: true}},
		{[]string{"dlv", "--headless", "exec"}, &dlvSpec{mode: "exec", headless: true, apiVersion: 2, log: false, onlySameUser: true}},
		{[]string{"dlv", "--headless", "exec", "--", "--listen=host:4345"}, &dlvSpec{mode: "exec", headless: true, apiVersion: 2, log: false, onlySameUser: true}},
		{[]string{"dlv", "test", "--headless", "--listen=host:4345"}, &dlvSpec{mode: "test", host: "host", port: 4345, headless: true, apiVersion: 2, log: false, onlySameUser: true}},
		{[]string{"dlv", "debug", "--headless", "--api-version=1"}, &dlvSpec{mode: "debug", headless: true, apiVersion: 1, log: false, onlySameUser: true}},
		{[]string{"dlv", "debug", "--listen=host:4345", "--headless", "--api-version=2", "--log"}, &dlvSpec{mode: "debug", host: "host", port: 4345, headless: true, apiVersion: 2, log: true, onlySameUser: true}},
		{[]string{"dlv", "debug", "--only-same-user"}, &dlvSpec{mode: "debug", apiVersion: 2, onlySameUser: true}},
		{[]string{"dlv", "debug", "--only-same-user=true"}, &dlvSpec{mode: "debug", apiVersion: 2, onlySameUser: true}},
		{[]string{"dlv", "debug", "--only-same-user=false"}, &dlvSpec{mode: "debug", apiVersion: 2, onlySameUser: false}},
	}
...
func TestDlvTransformerApply(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			result: v1.Container{
				Command: []string{"/dbg/go/bin/dlv", "exec", "--headless", "--continue", "--accept-multiclient", "--listen=localhost:56268", "--api-version=2", "--only-same-user=false", "app", "--", "arg"},
		...
		},
		{
			...
			result: v1.Container{
				Args:  []string{"/dbg/go/bin/dlv", "exec", "--headless", "--continue", "--accept-multiclient", "--listen=localhost:56268", "--api-version=2", "--only-same-user=false", "app", "--", "arg"},
				Ports: []v1.ContainerPort{{Name: "dlv", ContainerPort: 56268}},
			},
			debugConfig: ContainerDebugConfiguration{Runtime: "go", Ports: map[string]uint32{"dlv": 56268}},
			image:       "go",
		},
		{
			...
			result: v1.Container{
				Command: []string{"/dbg/go/bin/dlv", "exec", "--headless", "--continue", "--accept-multiclient", "--listen=localhost:56268", "--api-version=2", "--only-same-user=false", "app", "--"},
				Args:    []string{"arg1", "arg2"},
				Ports:   []v1.ContainerPort{{Name: "dlv", ContainerPort: 56268}},
			},
			debugConfig: ContainerDebugConfiguration{Runtime: "go", Ports: map[string]uint32{"dlv": 56268}},
